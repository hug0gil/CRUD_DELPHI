...
procedure TFormFichaCliente.btnAceptarClick(Sender: TObject);
var
  regimen: string;
...
begin
  ...
  begin
    ...
    try
      ...
      pFIBQuery.SQL.Text := 'SELECT GEN_ID(GEN_NCODIGO_CLIENTE, 1) FROM rdb$database;';
      pFIBQuery.ExecQuery;
      ...
      case mode of
        0: // Añadir
          begin
            pFIBQuery.Close;
            pFIBQuery.SQL.Text := 'INSERT INTO CLIENTES (NCODIGO, CNOMBRE, CREG_FISCAL) VALUES (:NCODIGO, :CNOMBRE, :CREG_FISCAL)';
        ...
        1: // Actualizar
          begin
            ...
            pFIBQuery.SQL.Text := 'UPDATE CLIENTES SET CNOMBRE = :CNOMBRE, CREG_FISCAL = :CREG_FISCAL WHERE NCODIGO = :OLD_NCODIGO';
...
procedure TFormFichaCliente.btnCancelarClick(Sender: TObject);
begin
  Self.Close;
end;

constructor TFormFichaCliente.Create(AOwner: TComponent; Modo: Integer);
begin
  inherited Create(AOwner);
  mode := Modo;
end;

function TFormFichaCliente.TodoCorrecto: Boolean;
var
...
begin
  ...
  // Comprobación solo cuando estamos añadiendo un nuevo cliente (mode = 0)
  ...
  begin
    ...
    pFIBQueryComprobar.SQL.Text :=
      'SELECT COUNT(*) FROM CLIENTES WHERE NCODIGO = :codigo';
    pFIBQueryComprobar.ParamByName('codigo').AsString := edtCodigo.Text;

    pFIBQueryComprobar.ExecQuery;

    // Comprobar si ya existe un cliente con el código, solo si estamos añadiendo un nuevo cliente
    if pFIBQueryComprobar.Fields[0].AsInteger >= 1 then
    begin
      ShowMessage('Ya existe un cliente con ese código.');
      Result := False;
      Exit;
    end;
