...
type
  TFormFichaLineasAlbaran = class(TFormFichaBase)
    ...
    procedure medtCajasPiezasChange(Sender: TObject);
    procedure medtUnidadesPesoChange(Sender: TObject);
...
constructor TFormFichaLineasAlbaran.Create(AOwner: TComponent; Modo: Integer);
begin
  inherited Create(AOwner);
  mode := Modo;
end;

procedure TFormFichaLineasAlbaran.FormActivate(Sender: TObject);
begin
  inherited;
  if mode <> 0 then
    getFactorConversion;
...
procedure TFormFichaLineasAlbaran.FormCreate(Sender: TObject);
begin
    ...
    try
      ...
      case mode of
        0: // Insertar
          begin
            pFIBQuery.SQL.Text :=
              'INSERT INTO LINEAS_ALB (NCOD_ALBARAN, CCOD_ARTICULO, NCANTIDAD1, NCANTIDAD2, NPRECIO, NORDEN, NIVA, NRECARGO,NSUBTOTAL) '
        ...
        2: // Actualizar
          begin
            pFIBQuery.SQL.Text :=
              'UPDATE LINEAS_ALB SET CCOD_ARTICULO = :CCOD_ARTICULO, NCANTIDAD1 = :NCANTIDAD1, NCANTIDAD2 = :NCANTIDAD2, NPRECIO = :NPRECIO, NIVA = :NIVA, NRECARGO = :NRECARGO, NSUBTOTAL = :NSUBTOTAL WHERE NCOD_ALBARAN = :NCOD_ALBARAN AND NORDEN = :NORDEN';
            pFIBQuery.ParamByName('NCOD_ALBARAN').AsInteger := StrToInt
              (edtCodigo.Text);
            pFIBQuery.ParamByName('CCOD_ARTICULO').AsString :=
              cbbCodArticulo.Text;
...
function TFormFichaLineasAlbaran.TodoCorrecto: Boolean;
var
  Precio: Double;
begin
  Result := False;
...
procedure TFormFichaLineasAlbaran.calcularSubtotal;
begin
  if (medtUnidadesPeso.Text = '') then
    medtUnidadesPeso.Text := '1';
  if (medtCajasPiezas.Text = '') then
    medtCajasPiezas.Text := '1';
  if (medtPrecio.Text = '') then
    medtPrecio.Text := '1';
  ...
  case FactorConversion of
    1:
      begin
        ...
        begin
          if (medtUnidadesPeso.Text = '') or (medtUnidadesPeso.Text = '0') then
            medtUnidadesPeso.Text := '1';
        ...
        begin
          if (medtCajasPiezas.Text = '') or (medtCajasPiezas.Text = '0') then
            medtCajasPiezas.Text := '1';
...
procedure TFormFichaLineasAlbaran.calcularSubtotalGranel;
begin
  if (medtCajasPiezas.Text = '') or (medtCajasPiezas.Text = '0') then
    medtCajasPiezas.Text := '1';
  if (medtUnidadesPeso.Text = '') or (medtUnidadesPeso.Text = '0') then
...
procedure TFormFichaLineasAlbaran.cbbCodArticuloChange(Sender: TObject);
begin
...
procedure TFormFichaLineasAlbaran.getCodArticulos;
begin
  try
    pFIBQuery.Close;
    if not pFIBTransaction.InTransaction then
...
procedure TFormFichaLineasAlbaran.getFactorConversion;
begin
  try
    pFIBQuery.Close;
    if not pFIBTransaction.InTransaction then
...
function TFormFichaLineasAlbaran.getNombreArticulo: String;
begin
  Result := '';
...
procedure TFormFichaLineasAlbaran.medtUnidadesPesoChange(Sender: TObject);
begin
  inherited;
  if medtUnidadesPeso.Text = '' then
...
procedure TFormFichaLineasAlbaran.medtUnidadesPesoExit(Sender: TObject);
...
procedure TFormFichaLineasAlbaran.medtCajasPiezasChange(Sender: TObject);
begin
  inherited;
  if medtCajasPiezas.Text = '' then
...
procedure TFormFichaLineasAlbaran.medtCajasPiezasExit(Sender: TObject);
...
procedure TFormFichaLineasAlbaran.medtPrecioExit(Sender: TObject);
begin
  inherited;
...
procedure TFormFichaLineasAlbaran.medtPrecioKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFormFichaLineasAlbaran.medtUnidadesPesoKeyPress(Sender: TObject;
  var Key: Char);
begin
  // Solo permitir números, coma y la tecla de retroceso
  if not(Key in ['0' .. '9', #8, ',']) then
...
procedure TFormFichaLineasAlbaran.medtCajasPiezasKeyPress(Sender: TObject;
  var Key: Char);
begin
  // Solo permitir números, coma y la tecla de retroceso
  if not(Key in ['0' .. '9', #8]) then
