...
type
  TFormFichaLineasAlbaranVentas = class(TFormFichaBase)
    pnlFila2: TPanel;
    pnlPrecio: TPanel;
    lblPrecio: TLabel;
    pnlOrden: TPanel;
...
constructor TFormFichaLineasAlbaranVentas.Create(AOwner: TComponent;
  Modo: Integer);
begin
...
procedure TFormFichaLineasAlbaranVentas.FormActivate(Sender: TObject);
begin
  inherited;
  if mode <> 0 then
    getFactorConversion;
...
procedure TFormFichaLineasAlbaranVentas.FormCreate(Sender: TObject);
begin
...
procedure TFormFichaLineasAlbaranVentas.btnAceptarClick(Sender: TObject);
begin
...
function TFormFichaLineasAlbaranVentas.TodoCorrecto: Boolean;
var
  Precio: Double;
begin
  Result := False;
  ...
  begin
    ...
    else if (medtCajasPiezas.Text = '') or (medtCajasPiezas.Text = '0') then
      medtCajasPiezas.Text := '1';
...
procedure TFormFichaLineasAlbaranVentas.calcularSubtotal;
begin
...
procedure TFormFichaLineasAlbaranVentas.calcularSubtotalGranel;
begin
  begin
...
procedure TFormFichaLineasAlbaranVentas.cbbCodArticuloChange(Sender: TObject);
begin
...
procedure TFormFichaLineasAlbaranVentas.getCodArticulos;
begin
  try
    pFIBQuery.Close;
    if not pFIBTransaction.InTransaction then
...
procedure TFormFichaLineasAlbaranVentas.getFactorConversion;
begin
  try
    pFIBQuery.Close;
    if not pFIBTransaction.InTransaction then
...
function TFormFichaLineasAlbaranVentas.getNombreArticulo: String;
begin
  Result := '';
...
procedure TFormFichaLineasAlbaranVentas.medtUnidadesPesoExit(Sender: TObject);
...
procedure TFormFichaLineasAlbaranVentas.medtCajasPiezasExit(Sender: TObject);
...
procedure TFormFichaLineasAlbaranVentas.medtPrecioExit(Sender: TObject);
begin
  inherited;
...
procedure TFormFichaLineasAlbaranVentas.medtPrecioKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFormFichaLineasAlbaranVentas.medtUnidadesPesoKeyPress
...
procedure TFormFichaLineasAlbaranVentas.medtCajasPiezasKeyPress
  (Sender: TObject; var Key: Char);
begin
  // Solo permitir números, coma y la tecla de retroceso
  if not(Key in ['0' .. '9', #8]) then
