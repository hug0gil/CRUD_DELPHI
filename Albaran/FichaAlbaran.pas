...
type
  TFormFichaGridAlbaran = class(TFormFichaBase)
    PanelFecha: TPanel;
    LabelFecha: TLabel;
...
constructor TFormFichaGridAlbaran.Create(AOwner: TComponent; LastCode: Integer;
  mode: Integer);
begin
...
function TFormFichaGridAlbaran.TodoCorrecto: Boolean;
var
  correcto: Boolean;
begin
  correcto := True;
  ...
  // Verificar que el código de cliente es válido
  if ComboBoxCodCliente.ItemIndex = -1 then
  begin
    MessageDlg('Seleccione un código de cliente válido.', mtError, [mbOK], 0);
    correcto := False;
    Exit;
  end;
  Result := correcto;
