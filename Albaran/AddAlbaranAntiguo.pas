...
type
  TFormAddAlbaranAntiguo = class(TForm)
    LabelCodigo: TLabel;
    LabelFecha: TLabel;
...
var
  FormAddAlbaranAntiguo: TFormAddAlbaranAntiguo;

implementation

{$R *.dfm}

procedure TFormAddAlbaranAntiguo.ButtonAddClick(Sender: TObject);
var
  QueryInsertar: TFDQuery;
  FechaFormateada: string;
begin
  // Comprobar que todos los campos necesarios están completos
  if (ComboBoxCliente.Text = '') or (MemoObservaciones.Lines.Text = '') then
  begin
  ...
  try
    ...
    QueryInsertar.ParamByName('Observaciones').AsString :=
      MemoObservaciones.Lines.Text;
    QueryInsertar.ParamByName('Cliente').AsInteger :=
      StrToInt(ComboBoxCliente.Text);

    // Ejecutar la consulta
    QueryInsertar.ExecSQL;

    // Mostrar mensaje de éxito con título personalizado
    MessageDlgPosHelp('El albarán ha sido añadido correctamente.',
      mtInformation, [mbOK], 0, -1, -1, '¡Éxito!');
...
procedure TFormAddAlbaranAntiguo.ButtonCancelarClick(Sender: TObject);
begin
  Self.Close;
end;

procedure TFormAddAlbaranAntiguo.FormCreate(Sender: TObject);
var
